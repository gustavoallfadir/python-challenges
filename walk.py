...
letter (direction) and you know it takes you one minute to traverse one city block, so 
...
def is_valid_walk(walk):
    ...
    coord = [0,0]
    for step in walk:
        coord[0] += steps[step][0]
        coord[1] += steps[step][1]
    return len(walk) == 10 and coord == [0,0]
